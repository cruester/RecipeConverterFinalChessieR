...
public  class Recipe {
    ...
    public void formatAsFraction() {
        ...
        for (int i = 0; i < counter; i++) {
            ...
            double numerator1 = 1; //Where actual numerator will be stored
            double numerator2 = 0;
            double denominator1 = 0; //Where actual denominator will be stored
            double denominator2 = 1;
            double b = x;

            do {
                double a = Math.floor(b); //Find the largest int that's <= newAmounts val
            ...
            /* We want the result of our decimal val minus the calculated numerator / denominator
              to be zero. That means they're equal. The tolerance is there in case we can't get
              them exactly equal. If the result isn't close enough to zero, we must continue
              the process*/


            if (x < .5) {
                //We need to simplify fractions < 1/2
                ...
                //Values between .5 and 1 don't need special modification
                int numerator1Int = (int) (numerator1);
                int denominator1Int = (int) (denominator1);
                amountsInFractionFormat.add(numerator1Int + "/" + denominator1Int);
            } else {
                ...
                int numToSubtract = numerator1Int - modNum; //numToSubtract yields a num divisible
                // by the denominator

                int wholeNum = numToSubtract / denominator1Int; //Whole num yields the whole num in a mixed fraction
                numerator1Int = numerator1Int - numToSubtract; //Subtract the whole number from the numerator
                // and you're left with the proper numerator
                if (numerator1Int == 0) {
                    amountsInFractionFormat.add(wholeNum + ""); //If the numerator divided evenly, there's no fraction
                } else {
